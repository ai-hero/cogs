...
def load_prompt():
    """Load the prompt"""
    if os.path.exists(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "instructions.prompt")
    ):
        with open(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "instructions.prompt"
            ),
            "r",
            encoding="utf-8",
        ) as file:
        ...
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "purpose.prompt")
    ):
        with open(
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "purpose.prompt"),
            "r",
...
def chat(prompt, skills_db):
    ...
    while True:
        ...
        print(f"\t\t\t\tFetch skills: {time.perf_counter() - stopwatch}")

        if len(relevant_skills) > 0:
            stopwatch = time.perf_counter()
            will_help = will_it_help(user_input, relevant_skills[0]["description"])
            print(f"\t\t\t\tWill it help: {time.perf_counter() - stopwatch}")
            if will_help:
                stopwatch = time.perf_counter()
                response = openai.ChatCompletion.create(
                    model="gpt-3.5-turbo", messages=messages, functions=relevant_skills
                )
                print(
                    f"\t\t\t\tSelect function skills: {time.perf_counter() - stopwatch}"
                )
                if (
                    ...
                    resp = skills_db.execute_command(
                        to_call["name"], json.loads(to_call["arguments"])
                    )
                    ...
                    print(f"\t\t\t\tCompletion: {time.perf_counter() - stopwatch}")
                    print(
                        "Agent (using skills):",
...
def update_prompt(prompt, conversation, feedback):
    ...
    if os.path.exists(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "reflect.prompt")
    ):
        while True:  # Loop to continuously improve the prompt based on user feedback
            with open(
                os.path.join(
                    os.path.dirname(os.path.abspath(__file__)), "reflect.prompt"
                ),
                "r",
                encoding="utf-8",
            ) as file:
            ...
            messages = [
                ...
                {
                    "role": "user",
                    "content": f"Original Instructions:\n{prompt}\nLast Conversation:\n{conversation}\nFeedback:\n{feedback}",
                },
            ]
            ...
            user_input = (
                input("Is this new prompt up to your liking? (yes/no): ")
                .strip()
                .lower()
            )
            if user_input.lower() == "yes":
                ...
                feedback = input(
                    "Please provide additional feedback to refine the prompt: "
                ).strip()
...
def will_it_help(question, function_description):
    messages = [
        {
            "role": "user",
            "content": f"Will the python function '{function_description}' help me with '{question}'? ONLY ANSWER YES OR NO.",
        }
    ]
    response = openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=messages)
